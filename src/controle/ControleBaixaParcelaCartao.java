 ...
 * @author cyro
 */
public class ControleBaixaParcelaCartao {
    
     ConectaBanco conecta = new ConectaBanco();
     ConectaBanco c = new ConectaBanco();
     SimpleDateFormat hoje = new SimpleDateFormat("dd/MM/yyyy");
//     public ModeloBaixaParcelaCartao BuscaParcela(ModeloBaixaParcelaCartao mod ){
//         
//         conecta.conexao();
//         
//         
...
public void BaixarParcela(ModeloBaixaParcelaCartao mod){
         conecta.conexao();
  
         try {
             
             
             conecta.executaSQL("SELECT * FROM parcela_cartao WHERE id_parc_venda ="+mod.getCodParc()+" AND estado = 'Pago'");
             if(conecta.rs.first()){
                ...
                try {
                 PreparedStatement pst = conecta.conn.prepareStatement("UPDATE parcela_cartao SET estado=?,  recebido=?, datarecebimento = ? WHERE id_parc_venda=?");
                 pst.setString(1, "Pago");
                 pst.setFloat(2, mod.getPago());
                 pst.setDate(3, new java.sql.Date(System.currentTimeMillis()));
                 pst.setInt(4, mod.getCodParc());              
                 pst.execute();
                 
                 c.conexao();
                PreparedStatement ps = c.conn.prepareStatement("INSERT INTO tbl_entrada_cartao (id_caixa, valor_pago,data_recebimento,id_venda)VALUES(?,?,?,?)");
